...
import javacore.orientacaoobjetos.Xstreams.dominio.Category;
import javacore.orientacaoobjetos.Xstreams.dominio.LigthNovel;
import javacore.orientacaoobjetos.Xstreams.dominio.Promotion;
...
public class StreamTest13 {
    ...
    public static void main(String[] args) {
        Map<Promotion, List<LigthNovel>> collect = ligthNovels
                .stream()
                .collect(Collectors.groupingBy(ln -> ln.getPrice() < 8 ? Promotion.UNDER_PORMOTION : Promotion.NORMAL_PRICE));

        System.out.println(collect);

        Map<Category, Map<Promotion, List<LigthNovel>>> collect1 = ligthNovels.stream()
                .collect(Collectors.groupingBy(LigthNovel::getCategory,
                        Collectors.groupingBy(ln -> ln.getPrice() < 8 ? Promotion.UNDER_PORMOTION : Promotion.NORMAL_PRICE)));
