...
public class BlockingQueueTest01 {
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.printf("%s added the named value %s%n", Thread.currentThread().getName(), bq.peek());
        System.out.println("Trying to add another value");
        new Thread(new RemoveFromQueue(bq)).start();

        bq.put("Alejandro");
        System.out.printf("%s added the named value %s%n", Thread.currentThread().getName(),bq.peek());//Ele espera o nome anterior sair
    }
